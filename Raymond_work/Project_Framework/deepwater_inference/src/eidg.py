...
def random_channel_shift(x, intensity=.0, channel_index=0):
...
def random_intensity_shift(x, intensity):

    min_x, max_x = np.min(x), np.max(x)
    sigma = intensity * (max_x - min_x) / 3
    x = x + np.random.normal(0, sigma)
    x = np.maximum(x, min_x)
    x = np.minimum(x, max_x)
    return x
...
def random_transform(x,
    ...
    # Arguments
        ...
        intensity_range: maximum difference of intensity, as fraction (0 to 1).
    ...
    if intensity_range != 0:
        x = random_intensity_shift(x, intensity_range)
