...
import torch
from dataset import JNet_Cells,get_isbi_filenames,validation_split,is_iterable
...
parser.add_argument('--dataset_root',default="/content/gdrive/My Drive/comp9517/project/jnet-master/toyexample/DIC-C2DH-HeLa_training", required=False, type=str,help='Directory with the dataset (it contains subdirectories 01, 02 and optionally 01_GT and 02_GT)') 
...
if args.mode is Mode.Train:
  ...
  train_set=JNet_Cells(training_images,training_gt,args.dt_bound,ast.literal_eval(args.resolution_levels),
  ...
  validation_set=JNet_Cells(validation_images,validation_gt,args.dt_bound,ast.literal_eval(args.resolution_levels),load_to_memory=bool(args.load_dataset_to_ram))
  ...
  dataset=JNet_Cells(image_filenames,gt_filenames,args.dt_bound,ast.literal_eval(args.resolution_levels),load_to_memory=bool(args.load_dataset_to_ram))
  evaluate(model,dataset,args)
elif args.mode is Mode.Vis:
  dataset=JNet_Cells(image_filenames,[],args.dt_bound,ast.literal_eval(args.resolution_levels),load_to_memory=bool(args.load_dataset_to_ram))
  evaluate(model,dataset,args)
else:
